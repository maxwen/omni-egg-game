...
public class EggGame {
	...
	final private static int DEFAULT_NUMBER_OF_LIVES = 5;
	...
	final private static int TIME_PER_LEVEL = 15000;
	...
	private void startCountdown() {
		...
		countdownRunnable = new Runnable(){
			...
			public void run() {
				...
				if(countdown>0){
					//soundHandler.playSoundEffect(SoundHandler.SOUND_COUNTDOWN, 50);
					...
					//soundHandler.playSoundEffect(SoundHandler.SOUND_GO, 50);
...
//		Log.d("Delay Egg Fall", "Egg fall delay: " +String.valueOf(delayEggFall));
	...
	protected void setMyAnimListener(Animation eggAnimation, final int pos,
		...
		animWhiteListener = new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				if(handlerStarted){//condition prevents showing of broken egg
                    gameGraphics.recycleEgg(eggView);
					checkIfScored(pos, 1);
